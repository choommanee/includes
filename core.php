...
function SK_getFollowSuggestions($search_query='', $limit=10) {
    if (!SK_isLogged()) {
        return false;
    }
    ...
    $query_one = "SELECT id FROM " . DB_ACCOUNTS . " WHERE id NOT IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=" . $user['id'] . ") AND (id IN (SELECT id FROM " . DB_USERS . " WHERE follow_privacy='everyone')) AND type IN ('user') AND active=1";
...
function SK_getPageSuggestions($search_query='', $limit=10) {
    if (!SK_isLogged()) {
        return false;
    }
    ...
    $limit = SK_secureEncode($limit);
    $query_one = "SELECT id FROM " . DB_ACCOUNTS . " WHERE id NOT IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=" . $user['id'] . ") AND (id IN (SELECT id FROM " . DB_PAGES . ")) AND type IN ('page') AND active=1";
...
function SK_getGroupSuggestions($search_query='', $limit=10) {
    if (!SK_isLogged()) {
        return false;
    }
    ...
    $limit = SK_secureEncode($limit);
    $query_one = "SELECT id FROM " . DB_ACCOUNTS . " WHERE id NOT IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=" . $user['id'] . ") AND (id IN (SELECT id FROM " . DB_GROUPS . " WHERE group_privacy IN ('open','closed'))) AND type IN ('group') AND active=1";
...
function SK_getFriends($timeline_id=0, $search_query='') {
    global $dbConnect, $user;
    $get = array();
    ...
    $timeline_id = SK_secureEncode($timeline_id);
    $query_text = "SELECT id FROM " . DB_ACCOUNTS . " WHERE id IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=$timeline_id AND following_id<>$timeline_id AND active=1) AND id IN (SELECT follower_id FROM " . DB_FOLLOWERS . " WHERE follower_id<>$timeline_id AND following_id=$timeline_id AND active=1) AND type='user' AND active=1";
...
function SK_getSearch($search_query='', $from_row=0, $limit=10) {
	...
	// 
    $query_one = "SELECT id FROM " . DB_POSTS . " WHERE youtube_title LIKE '%$search_query%' ORDER BY youtube_title ASC LIMIT $from_row,$limit";
    $sql_query_one = mysqli_query($dbConnect, $query_one);
    
    if (mysqli_num_rows($sql_query_one) > 0) {
        
        while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {			
			$get[] = SK_getStory($sql_fetch_one['id'], true);
        }
    }
	//
    return $get;
...
function SK_getChannel($search_query='', $from_row=0, $limit=100) {
    global $dbConnect;
    $get = array();
    ...
    $from_row = SK_secureEncode($from_row);
    $limit = SK_secureEncode($limit);
    $query_one = "SELECT id FROM " . DB_ACCOUNTS . " WHERE name LIKE '$search_query%' AND (id IN (SELECT id FROM " . DB_USERS . ") OR id IN (SELECT id FROM " . DB_GROUPS . " WHERE group_privacy IN ('open','closed'))) AND type IN ('user','group') AND active=1 ORDER BY name ASC LIMIT $from_row,$limit";
    $sql_query_one = mysqli_query($dbConnect, $query_one);
...
function SK_getSocial($search_query='', $from_row=0, $limit=100) {
    global $dbConnect;
    $get = array();
...
function SK_getMessages($data=array()) {
    ...
    $query_one = "SELECT id,active,media_id,clip_id,recipient_id,seen,text,time,timeline_id,type1 FROM " . DB_POSTS . " WHERE active=1";
    ...
    while ($sql_fetch = mysqli_fetch_assoc($sql_query_two)) {
		...
		if (!empty($sql_fetch['clip_id']) && $sql_fetch['clip_id'] > 0) {
            $sql_fetch['clip'] = SK_getMedia($sql_fetch['clip_id']);
        }
    ...
    if (isset($sk_publisher['id'])) {
        ...
        if ($sk_publisher['type'] == "user") {
            switch ($data['type']) {
				...
				case 'clips':
                    $query_text .= " AND timeline_id=" . $data['publisher_id'] . " AND recipient_id=0 AND clip_id>0 AND hidden=0 AND type1='story' AND type2='none'";
                break;
            ...
            switch ($data['type']) {
				...
				case 'clips':
                    $query_text .= " AND timeline_id=" . $data['publisher_id'] . " AND clip_id>0 AND hidden=0 AND type1='story' AND type2='none'";
                break;
            ...
            switch ($data['type']) {
				...
				case 'clips':
                    $query_text .= " AND recipient_id=" . $data['publisher_id'] . " AND clip_id>0 AND hidden=0 AND type1='story' AND type2='none'";
                break;
        ...
        switch ($data['type']) {
			...
			case 'clips':
                $query_text .= " AND (timeline_id IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=" . $user['id'] . " AND active=1) OR recipient_id IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=" . $user['id'] . " AND following_id IN (SELECT id FROM " . DB_GROUPS . "))) AND clip_id>0 AND hidden=0 AND type1='story' AND type2='none'";
            break;
...
function SK_getStory($story_id=0, $view_all_comments=false) {
    ...
    $query_one = "SELECT id,active,google_map_name,hidden,link_title,link_url,media_id,clip_id,post_id,recipient_id,soundcloud_title,soundcloud_uri,text,time,timeline_id,type1,type2,youtube_video_id,youtube_title,youtube_description,view FROM " . DB_POSTS . " WHERE id=$story_id AND type1='story' AND active=1";
    $sql_query_one = mysqli_query($dbConnect, $query_one);
    ...
    if (mysqli_num_rows($sql_query_one) == 1) {
        ...
        if ($post['id'] == $post['post_id']) { 
            ...
            $query_two = "SELECT id,active,google_map_name,hidden,link_title,link_url,media_id,clip_id,post_id,recipient_id,soundcloud_title,soundcloud_uri,text,time,timeline_id,type1,type2,youtube_video_id,youtube_title,youtube_description,view FROM " . DB_POSTS . " WHERE id=" . $post['post_id'] . " AND type1='story' AND type2='none' AND active=1";
            $sql_query_two = mysqli_query($dbConnect, $query_two);
        ...
        // Media (Photos || Clip || Youtube || Soundcloud)
        $story['media_exists'] = false;
        $story['media_type'] = false;
        $query_three = "SELECT id FROM " . DB_MEDIA . " WHERE id=" . $story['media_id'] . " AND active=1";
        $sql_query_three = mysqli_query($dbConnect, $query_three);
		
		$query_three_c = "SELECT id FROM " . DB_MEDIA . " WHERE id=" . $story['clip_id'] . " AND active=1";
        $sql_query_three_c = mysqli_query($dbConnect, $query_three_c);
        ...
        } elseif ($story['clip_id'] > 0 && mysqli_num_rows($sql_query_three_c) > 0) {
            $story['media_exists'] = true;
            $story['media_type'] = 'clips';
            
            $query_four = "SELECT id,type FROM " . DB_MEDIA . " WHERE id=" . $story['clip_id'] . " AND active=1";
            $sql_query_four = mysqli_query($dbConnect, $query_four);
            $sql_fetch_four = mysqli_fetch_assoc($sql_query_four);
            
            if ($sql_fetch_four['type'] == "clip") {
                $sql_fetch_four = SK_getMedia($sql_fetch_four['id']);
                $story['media_num'] = 1;
...
function SK_countFriends($timeline_id=0) {
    global $dbConnect, $user;
    ...
    $timeline_id = SK_secureEncode($timeline_id);
    $query = "SELECT id FROM " . DB_ACCOUNTS . " WHERE id IN (SELECT following_id FROM " . DB_FOLLOWERS . " WHERE follower_id=$timeline_id AND following_id<>$timeline_id AND active=1) AND id IN (SELECT follower_id FROM " . DB_FOLLOWERS . " WHERE following_id=$timeline_id AND follower_id<>$timeline_id AND active=1) AND type='user' AND active=1";
    $sql_query = mysqli_query($dbConnect, $query);
...
function SK_registerPost($data=array()) {
    ...
    if (isset($data['photos']['name'])) {
    ...
    }elseif (isset($data['clips']['name'])) {
		if (!empty($data['clip_title'])) {
            $youtube_title = SK_secureEncode($data['clip_title']);
			$youtube_description = SK_secureEncode($data['clip_description']);
        }
        if (count($data['clips']['name']) == 1) {
            $clip_param = array(
                'tmp_name' => $data['clips']['tmp_name'][0],
                'name' => $data['clips']['name'][0],
                'size' => $data['clips']['size'][0]
            );
            $clip_data = SK_registerMediaClip($clip_param);
            ...
            $query_one = "INSERT INTO " . DB_MEDIA . " (timeline_id,active,name,type) VALUES (" . $timeline['id'] . ",1,'temp_" . SK_generateKey() . "','clip')";
            $sql_query_one = mysqli_query($dbConnect, $query_one);
            ...
            if ($sql_query_one) {
                ...
                for ($i = 0; $i < count($data['clips']['name']); $i++) {
                    ...
                    $clip_data = SK_registerMediaClip($clip_param, $clip_id);
        ...
        if (!empty($data['youtube_title'])) {
			...
			$api_url = 'http://gdata.youtube.com/feeds/api/videos/' . $youtube_video_id . '?v=2&alt=json&prettyprint=true';
			$api_content = @file_get_contents($api_url);
			$api_content_arr = json_decode($api_content, true);	

			//$youtube_description = $data['youtube_description'];
			$youtube_description = SK_secureEncode($api_content_arr['entry']['media$group']['media$description']['$t']);
        }
    }
    ...
    if ($post_ability == true) {
        $query_one = "INSERT INTO " . DB_POSTS . " (active,google_map_name,media_id,clip_id,soundcloud_title,soundcloud_uri,text,time,timeline_id,recipient_id,type1,type2,youtube_video_id,youtube_title,youtube_description) VALUES (1,'$google_map_name',$media_id,$clip_id,'$soundcloud_title','$soundcloud_uri','$text'," . time() . "," . $timeline['id'] . ",$recipient_id,'$type1','$type2','$youtube_video_id','$youtube_title','$youtube_description')";
        $sql_query_one = mysqli_query($dbConnect, $query_one);
            ...
            if (preg_match('/(jpg|jpeg|png)/', $ext)) {
                ...
                if ($sql_query_one) {
                    ...
                    if (move_uploaded_file($upload['tmp_name'], $original_file)) {
                        ...
                        SK_processMedia('resize', $original_file, $original_file, $cover, 0);                        
...
/* Clip upload */
function SK_registerMediaClip($upload, $clip_id=0) {
    if (!SK_isLogged()) {
        return false;
    }
    ...
    if (!file_exists('clips/' . date('Y'))) {
        mkdir('clips/' . date('Y'), 0755, true);
    }
    
    if (!file_exists('clips/' . date('Y') . '/' . date('m'))) {
        mkdir('clips/' . date('Y') . '/' . date('m'), 0755, true);
    }
    ...
    if (is_uploaded_file($upload['tmp_name'])) {
        ...
        if ($upload['size'] > 1024) {
            ...
            if (preg_match('/(mp4|flv)/', $ext)) { 
                $query_one = "INSERT INTO " . DB_MEDIA . " (extension,name,type) VALUES ('$ext','$name','clip')";
                $sql_query_one = mysqli_query($dbConnect, $query_one);
                
                if ($sql_query_one) {
                    $sql_id = mysqli_insert_id($dbConnect);
                    $original_file_name = $clip_dir . '/' . SK_generateKey() . '_' . $sql_id . '_' . md5($sql_id);
                    $original_file = $original_file_name . '.' . $ext;
                    
                    if (move_uploaded_file($upload['tmp_name'], $original_file)) {                       
                        if (!file_exists('clips/thumbs')) {
							mkdir('clips/thumbs', 0755, true);
						}
						$clip_thumb_dir = 'clips/thumbs';
						$cmd = "/usr/bin/ffmpeg -i /home/vdomax/" . $original_file ." -deinterlace -an -ss 1 -t 00:00:01 -r 1 -y -vcodec mjpeg -f mjpeg /home/vdomax/".$clip_thumb_dir."/media_clip_".$sql_id.".jpeg 2>&1";
						exec($cmd);
						mysqli_query($dbConnect, "UPDATE " . DB_MEDIA . " SET clip_id=$clip_id,url='$original_file_name',temp=0,active=1 WHERE id=$sql_id");                       
                        $get = array(
                            'id' => $sql_id,
                            'active' => 1,
                            'extension' => $ext,
...
function SK_livechannel($name){
	$xmlstr = file_get_contents("http://150.107.31.6:8080/stat");
	$ret = array();
	$xmlcont = new SimpleXMLElement($xmlstr);	
	foreach($xmlcont->server->application->live->stream as $para) {	
		if($para->name == $name){
			$ret['status'] = 200;
			$ret['visitors'] = $para->nclients;
			return $ret;
		}else{
			$ret['status'] = 404;
			$ret['visitors'] = 0;			
...
function SK_getRandStory($num){
	global $dbConnect;        
    
    $query_one = "SELECT id,active,google_map_name,hidden,link_title,link_url,media_id,clip_id,post_id,recipient_id,soundcloud_title,soundcloud_uri,text,time,timeline_id,type1,type2,youtube_video_id,youtube_title,youtube_description,view FROM " . DB_POSTS . " WHERE ((media_id != 0 OR media_id > 0) OR youtube_video_id != '') AND type1='story' AND active=1 AND hidden = 0 ORDER BY RAND() LIMIT ".$num;
    $sql_query_one = mysqli_query($dbConnect, $query_one);
...
function SK_getVideos($query,$numL = 0,$numM = 50){
	global $dbConnect;
	$sql = '';
	if(!empty($query)){
		$query = SK_secureEncode($query);
		$sql = "AND youtube_title LIKE '%".$query."%'";
    }
	$query_one = "SELECT id,active,google_map_name,hidden,link_title,link_url,media_id,clip_id,post_id,recipient_id,soundcloud_title,soundcloud_uri,text,time,timeline_id,type1,type2,youtube_video_id,youtube_title,youtube_description,view FROM " . DB_POSTS . " WHERE ((clip_id != 0 OR clip_id > 0) OR youtube_video_id != '') AND type1='story' AND active=1 AND hidden = 0 ".$sql." LIMIT ".$numL.", ".$numM;
    $sql_query_one = mysqli_query($dbConnect, $query_one);
    
    if (mysqli_num_rows($sql_query_one) > 0) {
		while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
			$videos[] = $sql_fetch_one;
		}
	}
	return $videos;
...
function SK_getPhotos($numL = 0,$numM = 50){
	global $dbConnect;	
	$query_one = "SELECT id FROM " . DB_POSTS . " WHERE media_id > 0 AND type1='story' AND active=1 AND hidden = 0 ORDER BY id DESC LIMIT ".$numL.", ".$numM;    
	$sql_query_one = mysqli_query($dbConnect, $query_one);   
    if (mysqli_num_rows($sql_query_one) > 0) {
		while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
			$photos[] = $sql_fetch_one;
		}
	}
	return $photos;	
}

function SK_countPhotos(){
	global $dbConnect;	
	$query_one = "SELECT COUNT(id) AS rows FROM " . DB_POSTS . " WHERE media_id > 0 AND type1='story' AND active=1 AND hidden = 0";    
	$sql_query_one = mysqli_query($dbConnect, $query_one);
   
    if (mysqli_num_rows($sql_query_one) > 0) {
		while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
			$row = $sql_fetch_one['rows'];
		}
	}
	return $row;	
}

function SK_videoViewUpdate($id){
	global $dbConnect;        
    $post = array();
    $query_one = "UPDATE " . DB_POSTS . " SET view=view+1 WHERE id = ".$id;
    $sql_query_one = mysqli_query($dbConnect, $query_one);
    if($sql_query_one){
		$post['status'] = 200;
		$post['id'] = $id;
...
function send_mail($email,$subject,$msg) {
	$api_key="key-289bd2ea4e3c23feafc75d075785d65f";/* Api Key got from https://mailgun.com/cp/my_account */
	$domain ="vdomax.com";/* Domain Name you given to Mailgun */
	$ch = curl_init();
	...
	curl_setopt($ch, CURLOPT_POSTFIELDS, array(
		'from' => 'VDOMax <noreply@vdomax.com>',
		'to' => $email,
		'subject' => $subject,
		'html' => $msg
	));
...
function getUserID($username){
	global $dbConnect;
	$query = "SELECT id FROM " . DB_ACCOUNTS . " WHERE username LIKE '" . $username . "' AND type IN ('user','page','group') AND active=1 LIMIT 1";
	$sql_query = mysqli_query($dbConnect, $query);
...
/* Register functions */
function SK_liveNotification($data=array()) {
